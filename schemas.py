...
class SchemaGenerator(object):
    ...
    def get_link(self, path, method, view):
        ...
        fields += self.get_proj_fields(path, method, view)
    ...
    def get_query_fields(self, path, method, view):
        if not is_list_view(path, method, view):
            return []
        if not getattr(view, 'query_class', None):
            return []
        queryclass = view.query_class()
        return queryclass.get_schema_fields(view)

    def get_proj_fields(self, path, method, view):
        if not is_list_view(path, method, view):
            return []
        if not getattr(view, 'proj_class', None):
            return []
        projectionclass = view.proj_class()
        return projectionclass.get_schema_fields(view)
